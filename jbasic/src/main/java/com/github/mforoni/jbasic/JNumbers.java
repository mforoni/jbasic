...
/**
 * Provides some {@code static} utility methods for dealing with {@code Number} objects, performing operations like
 * comparing and adding two numbers.
 * 
 * @author Foroni
 * @see Number
 */
public final class JNumbers {
	...
	/**
	 * {@link Comparator} for {@code Number} objects. Performs {@link JNumbers#compare(Number, Number)}.
	 */
	...
	/**
	 * Compares the two given {@code Number} objects. The numbers are converted to {@code BigDecimal} and then compared
	 * performing {@link BigDecimal#compareTo(BigDecimal)}.
	 * 
	 * @param n1
	 *            the first non-null {@code Number}
	 * @param n2
	 *            the second non-null {@code Number}
	 * @return -1, 0, or 1 as {@code n1} is numerically less than, equal to, or greater than {@code n2}.
	 * @see Number
	 * @see BigDecimal
	...
	/**
	 * Returns <tt>true</tt> if the two specified {@code Number} objects are equals, otherwise returns <tt>false</tt>.
	 * 
	 * @param n1
	 *            the first {@code Number}
	 * @param n2
	 *            the second {@code Number}
	 * @return <tt>true</tt> if the two specified {@code Number} objects are equals, otherwise returns <tt>false</tt>
	 * @see Number
	...
	public static Number add(@Nonnull final Number n1, @Nonnull final Number n2) {
		...
		if (n1 instanceof Integer && n2 instanceof Integer) {
			...
			throw new IllegalArgumentException("The arguments n1 and n2 must have the same type and the type must be a subclass of Number");
		}
	}
}